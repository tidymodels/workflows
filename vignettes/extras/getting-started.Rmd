...
```{r plot-data, message = FALSE, fig.alt = "A ggplot dotplot with predictor 'A' on the x axis and 'B' on the y, demonstrating a positive, linear, moderately strong correlation. Dots are colored according to two classes, red for 'One' and blue for 'Two', and the red dots tend to have slightly larger values of 'B' for given values of 'A', though the separation is far from perfect."}
library(workflows)
library(ggplot2)
library(dplyr)
...
```{r plot-marginals, fig.alt = "A pair of boxplots showing the same predictors, this time one at a time. The predictors on their own do little to separate the two outcome classes."}
library(tidyr)
...
```{r simple-roc, fig.alt = "A ggplot2 line plot, with 1 minus specificity on the x axis, sensitivity on the y, and a dotted line tracing the y equals x line. A solid line is shown always above the dotted line, with greatest separation from it towards the center of the plot."}
library(yardstick)
...
```{r ratios, fig.alt = "The same plot as before, this time with a yellow line also added to the plot. The yellow line roughly traces the solid black one, except that it's y values are slightly higher than those of the solid black line towards the left portion of the plot and vice versa for the other portion."}
ratio_glm <-
  glm_workflow %>%
...
```{r rec-trans, fig.alt = "The same plot as above, but with another line, shown in green, added to the plot. Its y values are nearly always consistently higher than the existing black and yellow lines."}
trans_glm <-
  glm_workflow %>%
...
```{r plot-inverse, fig.alt = "A dotplot like that shown early on in the article, plotting the inverse of the previous two predictors and the same binary outcome variable. A boundary between the two outcome classes seems a bit more evident in this plot than before."}
...
```{r test-set, fig.alt = "An ROC curve plot like before, back to showing just one solid black line, representing what had been shown as green in the previous plot. There's quite a bit of separation between this line and the identity, showing a model that's similarly as performant as it was on the training set."}
test_probs <- 
