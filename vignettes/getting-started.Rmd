...
This is an example of a fairly realistic interactive data analysis project and how workflows can be used. 

## Introduction

The data are simple: there are two predictors that should be used to differentiate two classes in the data. 

There are three partitions of the original data: one for training (n = 1009), validation (n = 300), and testing (n = 710). We will work with the training set the most and use the validation set to compare models during the development process and then use the test set once we think that we have a good algorithm for making predictions.  
...
Since there are two correlated predictors with skewed distributions and strictly positive values, it might be intuitive to use their ratio instead of the pair. We'll try that next but recycling the initial workflow and just adding a different formula:
...
autoplot(simple_glm_roc) + 
  geom_step(data = ratio_glm_roc %>% arrange(specificity), 
...
This can be easily done via the formula interface, so a recipe is used. A recipe is a list of sequential data processing steps that are conducted before the data are used in a model. For example, to use the Box-Cox method, a simple recipe would be:
...
Now we have all of the ingerdient to execute the data pre-processing. To get the pre-processed version of the training set, the `juice()` function can be used to return a data frame with the transformed predictors, and these can be given to the `fit()` function. 
...
autoplot(simple_glm_roc) + 
  geom_step(data = ratio_glm_roc %>% arrange(specificity), 
            aes(x = 1 - specificity, y = sensitivity),
              col = "darkred") + 
  geom_line(data = trans_glm_roc %>% arrange(specificity), 
            aes(x = 1 - specificity, y = sensitivity),
              col = "darkblue")
```

That is a potential, if not slight, improvmement. 

The Box-Cox procedure recommended transfomrations that are pretty close to the inverse (TODO have a way to access the `tidy` method on these objects).
...
autoplot(simple_glm_roc) + 
  ...
  geom_line(data = trans_glm_roc %>% arrange(specificity), 
            aes(x = 1 - specificity, y = sensitivity),
              col = "darkblue") + 
  geom_line(data = pca_glm_roc %>% arrange(specificity), 
            aes(x = 1 - specificity, y = sensitivity),
              col = "green")
```

These predictions are almost identical to the transformed model. 
...
Based on these results, the model with the logitic regression model with inverse terms is probably our best bet. Using the test set: 
